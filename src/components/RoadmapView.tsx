  ...
  const [weeks, setWeeks] = useState<WeekPlan[]>([
    ...
    },
    {
      ...
      milestones: [
        ...
        { id: 'w2-2', label: 'Build Shuffle SOAR webhook listener -> MISP lookup -> Wazuh Active Response', completed: true },
    ...
    {
      week: 3,
      title: 'Week 3: Zone 2 Target Grid Deployment',
      theme: 'Corporate Domain & Identity Federation (External Contributor)',
      zone: 'Zone 2 (Target Grid - 192.168.50.0/24)',
      deliverable: 'Target Grid Architecture (CORP-DC01, CORP-PC01, CORP-WEB01 Juice Shop & Keycloak ↔ AD Federation)',
      criteria: 'External contributor provisioning CORP-DC01 (Win Server 2022 AD DS primary enterprise identity store), joined CORP-PC01 ("Patient Zero" Win10 with Sysmon v15), deployed CORP-WEB01 (OWASP Juice Shop target app), and Keycloak ↔ Active Directory Federation via LDAP/OIDC sync; Wazuh Agents registered.',
      status: 'in-progress',
      milestones: [
        { id: 'w3-1', label: 'Promote CORP-DC01 to Active Directory Domain Controller (aegis.corp primary identity store)', completed: false },
        { id: 'w3-2', label: 'Join CORP-PC01 Windows 10 workstation to domain with Sysmon v15 (SwiftOnSecurity config)', completed: false },
        { id: 'w3-3', label: 'Configure Keycloak ↔ Active Directory Federation via LDAP/OIDC sync (CORP-DC01 identity bridge)', completed: false },
        { id: 'w3-4', label: 'Deploy CORP-WEB01 (OWASP Juice Shop target app) on 192.168.50.20 behind Coraza WAF', completed: true },
      ],
    },
    {
      week: 4,
      ...
      deliverable: 'Attack Framework (Atomic Red Team, Web App Exploitation SQLi/XSS, Directory Fuzzing, Credential Attacks)',
      criteria: 'Atomic Red Team scripts, web app exploits (SQLi & XSS against CORP-WEB01), directory fuzzing & path traversal (gobuster/ffuf), and credential attacks reliably trigger Coraza block and SOAR isolation; 5 full dry runs completed.',
      status: 'pending',
      milestones: [
        { id: 'w4-1', label: 'Script Red Team attack vectors: Atomic Red Team, Web App Exploitation (SQLi & XSS against CORP-WEB01), Directory Fuzzing & Path Traversal (gobuster/ffuf), Credential Brute Force & Spraying', completed: false },
        ...
        { id: 'w4-5', label: 'Atomic Red Team coverage testing — automated test battery execution', completed: false },
      ],
    },
  ]);
