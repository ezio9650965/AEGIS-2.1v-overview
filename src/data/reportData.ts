...
export const INITIAL_CHECKLIST_DONE: ChecklistItem[] = [
  ...
  { id: 'd18', title: 'Coraza WAF (Caddy + OWASP CRS)', description: 'Custom xcaddy build with OWASP CRS vendored to /srv. Found today that despite this task being marked done, Coraza was never actually in the traffic path: traefik-dynamic.yml routed the juiceshop service directly to 192.168.19.175:3000, bypassing Coraza entirely since original deployment. Fixed by repointing the service to http://coraza:8080. Also found and fixed a second, independent bug once traffic finally reached it: the Caddyfile used a single wildcard `Include /srv/*.conf`, which glob-sorts by byte value, loading REQUEST-*.conf files (uppercase, sorts first) before crs-setup.conf (lowercase) — meaning the CRS setup marker variable was never set in time, and CRS rejected every single request with a critical setup error since deployment. Fixed by explicitly ordering includes (crs-setup.conf first). Confirmed working: a real request now returns genuine Juice Shop HTML through the WAF with clean logs, no rule violations.', category: 'critical', completed: true, who: 'eagle' },
...
export const ZONE_STATUS = {
  ...
  summary: 'Zone 4 detection pipeline (Zeek/Suricata/Authelia -> Wazuh agent -> MITRE-tagged rules on minisoc2) verified end-to-end via wazuh-logtest as of September 13, 2026. Shuffle SOAR enrichment stage now verified working end-to-end: a real live Wazuh alert (T1055) flowed Wazuh -> Elasticsearch -> Logstash -> Shuffle -> MISP restSearch and returned a correct, precise match. Decision/response logic (branch + action) intentionally deferred. Outstanding: OpenLDAP pipeline (not started), decision/response node, Active Response integration (paused by design). Log ingestion audit performed across the full stack: found and fixed a Wazuh manager config gap (logall/logall_json disabled, meaning full-fidelity archives were never captured, only rule-matched alerts) and a Coraza WAF traffic-path gap (WAF was deployed but never actually in the request path since original setup, and its rules were failing silently once traffic did reach it due to an include-order bug). Both fixed and verified with real traffic today. Next: organize per-source Elasticsearch indices/dashboards now that full data is flowing, then begin controlled attack testing across all sources (Windows/Sysmon, Juice Shop, gateway auth).',
